...
function publicState(r,ws){
  ...
  return {room:r.code,host:ws.__id===r.hostId,slot,phase:r.phase,settings:r.settings,map:r.settings.map,players:Array.from({length:r.settings.players},(_,i)=>({slot:i,name:r.slots[i]?.name||`CPU ${i+1}`,human:!!r.slots[i],build:r.fight?undefined:r.slots[i]?.build??null,wins:r.match.wins[i]||0})),draft:r.draft,fight:r.fight?{elapsed:r.fight.elapsed,arenaHalf:r.fight.arenaHalf,entities:r.fight.entities.map(e=>({id:e.id,x:e.x,y:e.y,vx:e.vx,vy:e.vy,alive:e.alive,falling:e.falling,rotation:e.rotation,hits:e.hits,attack:e.attack?{kind:e.attack.kind,t:e.attack.t,total:e.attack.total,dir:e.attack.dir}:null,build:e.build,flash:e.flash}))}:null};
}
